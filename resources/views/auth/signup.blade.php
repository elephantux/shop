...
@section('content')
    <x-forms.auth title="Регистрация" action="{{ route('register.handle') }}" method="POST">

        <x-forms.text-input type="text" name="name" value="{{ old('name') }}" placeholder="Имя" required :is-error="$errors->has('name')"/>
        @error('name')
        ...
        <x-slot:links>
            <div class="space-y-3 mt-5">
                <div class="text-xxs md:text-xs">
                    <a href="{{ route('login') }}" class="text-white hover:text-white/70 font-bold">Войти в акканут</a>
                </div>
            </div>
        </x-slot:links>
